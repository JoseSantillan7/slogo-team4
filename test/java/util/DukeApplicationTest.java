...
public class DukeApplicationTest extends ApplicationTest {
  ...
  protected void clickOn(GridPane g, Node item) {
    ...
    simulateAction(offset.getX(), offset.getY(), () -> g.getOnMouseClicked().handle(
        new MouseEvent(MouseEvent.MOUSE_CLICKED, offset.getX(), offset.getY(), offset.getX(),
            offset.getY(), MouseButton.PRIMARY, 1, false, false, false, false, true, false, false,
            true, false, false, new PickResult(item, offset.getX(), offset.getY()))));
  }

  protected void clickOn(Node n, int x, int y) {
    Point2D offset = point(n).atPosition(Pos.TOP_LEFT).atOffset(new Point2D(x, y)).query();
    simulateAction(offset.getX(), offset.getY(), () -> n.getOnMouseClicked().handle(
        new MouseEvent(MouseEvent.MOUSE_CLICKED, offset.getX(), offset.getY(), offset.getX(),
            offset.getY(), MouseButton.PRIMARY, 1, false, false, false, false, true, false, false,
            true, false, false, new PickResult(n, x, y))));
  }
